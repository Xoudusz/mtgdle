...
export default function ArtGame({ cards, dailyCard, anchorX, anchorY }: Props) {
  ...
  function handleGuess(card: Card) {
    ...
    saveResult({ date: todayStr(), mode: 'art', solved: false, guesses: next.map(g => g.name), continued })
  ...
  function handleContinue() {
    setShowContinueModal(false)
    setContinued(true)
    saveResult({ date: todayStr(), mode: 'art', solved: false, guesses: guesses.map(g => g.name), continued: true })
  }

  const wrongGuesses = guesses.filter(g => !g.correct)
  const artUrl = dailyCard.image_uris?.art_crop ?? dailyCard.image_uris?.normal ?? ''
    ...
    <main className="min-h-screen px-4 py-8">
      <div className="w-full max-w-lg mx-auto flex flex-col items-center gap-6">
        <div className="text-center">
          <h1 className="text-3xl font-bold">Artdle</h1>
          <p className="text-[#9b8a6e] text-sm mt-1">Identify today's card from its artwork</p>
        </div>
        ...
        <ArtZoom
          artUrl={artUrl}
          guessCount={wrongGuesses.length}
          solved={solved}
          anchorX={anchorX}
          anchorY={anchorY}
        />
